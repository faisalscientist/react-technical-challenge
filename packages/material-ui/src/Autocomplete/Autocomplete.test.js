...
describe('<Autocomplete />', () => {
  ...
  describe('prop: autoSelect', () => {
    it('should not clear on blur when value does not match any option', () => {
      ...
      expect(onChange.args[0][1]).to.deep.equal('oo');
    });
    ...
    it('should add new value when autoSelect & multiple on blur', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal(options);
    });
    ...
    it('should add new value when autoSelect & multiple & freeSolo on blur', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal(['a']);
    });
  });
  ...
  describe('prop: multiple', () => {
    ...
    it('should remove the last option', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal([options[0]]);
    });
    ...
    it('navigates between different tags', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal([options[1]]);
  ...
  describe('warnings', () => {
    ...
    it('warn if getOptionLabel do not return a string', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.equal('a');
    });
  ...
  describe('prop: options', () => {
    it('should keep focus on selected option and not reset to top option when options updated', () => {
      ...
      // Important. Re-render with the same options, make sure don't break the scroll position
    ...
    it('should not select undefined', () => {
      ...
      expect(onChange.args[0][1]).to.equal('one');
    });
  ...
  describe('controlled', () => {
    it('controls the input value', () => {
      ...
      fireEvent.change(document.activeElement, { target: { value: 'a' } });
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][0]).to.equal('a');
    ...
    it('should fire the input change event before the change event', () => {
      ...
      expect(handleInputChange.calledBefore(onChange)).to.equal(true);
    });
  });
  ...
  describe('prop: freeSolo', () => {
    ...
    it('should not fire change event until the IME is confirmed', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.equal('あ');
    });
  });
  ...
  describe('prop: onChange', () => {
    it('provides a reason and details on option creation', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.equal(options[2]);
      expect(onChange.args[0][2]).to.equal('create-option');
      expect(onChange.args[0][3]).to.deep.equal({ option: options[2] });
    });
    ...
    it('provides a reason and details on option selection', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.equal(options[0]);
      expect(onChange.args[0][2]).to.equal('select-option');
    ...
    it('provides a reason and details on option removing', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal(options.slice(0, 2));
      expect(onChange.args[0][2]).to.equal('remove-option');
      expect(onChange.args[0][3]).to.deep.equal({ option: options[2] });
    });
    ...
    it('provides a reason and details on blur', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.equal(options[0]);
      expect(onChange.args[0][2]).to.equal('blur');
      expect(onChange.args[0][3]).to.deep.equal({ option: options[0] });
    });
    ...
    it('provides a reason and details on clear', () => {
      ...
      expect(onChange.callCount).to.equal(1);
      expect(onChange.args[0][1]).to.deep.equal([]);
      expect(onChange.args[0][2]).to.equal('clear');
  ...
  it('should filter options when new input value matches option', () => {
    ...
    fireEvent.keyDown(textbox, { key: 'Enter' });
    expect(onChange.callCount).to.equal(1);
    expect(onChange.args[0][1]).to.deep.equal('one');
    expect(combobox).to.have.attribute('aria-expanded', 'false');
